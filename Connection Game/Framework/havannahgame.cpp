...
HavannahGame::HavannahGame()
{

}

std::vector<Board::Position> HavannahGame::FindCorners(const HavannahBoard &board, Piece::Color playerColor)
{
    ...
    if (m_board.at(m_board.at(0).size() - 1).at(0).has_value()
        && m_board.at(m_board.at(0).size() - 1).at(0).value().GetColor() == playerColor)
    {
        corners.push_back(std::make_pair(m_board.at(0).size() - 1, 0));
    }

    if (m_board.at(m_board.size() - 1).at(0).has_value()
        && m_board.at(m_board.size() - 1).at(0).value().GetColor() == playerColor)
    {
        corners.push_back(std::make_pair(m_board.size() - 1, 0));
    }

    if (m_board.at(0).at(m_board.at(0).size() - 1).has_value()
        && m_board.at(0).at(m_board.at(0).size() - 1).value().GetColor() == playerColor)
    {
        corners.push_back(std::make_pair(0, m_board.at(0).size() - 1));
    }

    if (m_board.at(m_board.at(0).size() - 1).at(m_board.size() - 1).has_value()
        && m_board.at(m_board.at(0).size() - 1).at(m_board.size() - 1).value().GetColor() == playerColor)
    {
        corners.push_back(std::make_pair(m_board.at(0).size() - 1, m_board.size() - 1));
    }

    if (m_board.at(m_board.size() - 1).at(m_board.at(0).size() - 1).has_value()
        && m_board.at(m_board.size() - 1).at(m_board.at(0).size() - 1).value().GetColor() == playerColor)
    {
        corners.push_back(std::make_pair(m_board.size() - 1, m_board.at(0).size() - 1));
    }

    return corners;
}

bool HavannahGame::BFS_Bridge(const HavannahBoard &board, Piece::Color playerColor)
{
...
std::vector<Board::Position> HavannahGame::GetTopLeftEdge(const HavannahBoard &board, Piece::Color playerColor)
{
    std::vector<Board::Position> topLeftEdge;
    for (int index = 1; index < m_board.size() / 2; index++)
    {
        if (m_board.at(index).at(0).has_value()
            && m_board.at(index).at(0).value().GetColor() == playerColor)
        {
...
std::vector<Board::Position> HavannahGame::GetBottomLeftEdge(const HavannahBoard &board, Piece::Color playerColor)
{
    std::vector<Board::Position> bottomLeftEdge;
    for (int index = m_board.size() / 2 + 1; index < m_board.size() - 1; index++)
    {
        if (m_board.at(index).at(0).has_value()
            && m_board.at(index).at(0).value().GetColor() == playerColor)
        {
...
std::vector<Board::Position> HavannahGame::GetTopRightEdge(const HavannahBoard &board, Piece::Color playerColor)
{
    std::vector<Board::Position> topRightEdge;
    for (int index = 1; index < m_board.size() / 2; index++)
    {
        if (m_board.at(index).at(m_board.at(index).size() - 1).has_value()
            && m_board.at(index).at(m_board.at(index).size() - 1).value().GetColor() == playerColor)
        {
            topRightEdge.push_back(std::make_pair(index, m_board.at(index).size() - 1));
        }
    }
    return topRightEdge;
}

std::vector<Board::Position> HavannahGame::GetBottomRightEdge(const HavannahBoard &board, Piece::Color playerColor)
{
    ...
    for (int index = m_board.size() / 2 + 1; index < m_board.size() - 1; index++)
    {
        if (m_board.at(index).at(m_board.at(index).size() - 1).has_value()
            && m_board.at(index).at(m_board.at(index).size() - 1).value().GetColor() == playerColor)
        {
            bottomRightEdge.push_back(std::make_pair(index, m_board.at(index).size() - 1));
        }
    }
    return bottomRightEdge;
}

std::vector<Board::Position> HavannahGame::GetTopEdge(const HavannahBoard &board, Piece::Color playerColor)
{
    std::vector<Board::Position> topEdge;
    for (int index = 1; index < m_board.at(0).size() - 1; index++)
    {
        if (m_board.at(0).at(index).has_value()
            && m_board.at(0).at(index).value().GetColor() == playerColor)
        {
...
std::vector<Board::Position> HavannahGame::GetBottomEdge(const HavannahBoard &board, Piece::Color playerColor)
{
    std::vector<Board::Position> bottomEdge;
    for (int index = 1; index < m_board.at(0).size() - 1; index++)
    {
        if (m_board.at(m_board.size() - 1).at(index).has_value()
            && m_board.at(m_board.size() - 1).at(index).value().GetColor() == playerColor)
        {
            bottomEdge.push_back(std::make_pair(m_board.size() - 1, index));
        }
    }
    return bottomEdge;
}

bool HavannahGame::BFSBetweenTwoEdges(const HavannahBoard &board, std::vector<Board::Position> edge1, std::vector<Board::Position> edge2, Piece::Color playerColor)
{
    std::set<Board::Position> visited;
    std::queue<Board::Position> path;
...
std::vector<Board::Position> HavannahGame::FindTheEdge(const HavannahBoard &board, int edge, Piece::Color playerColor)
{
    switch (edge)
    {
    case 0:
...
bool HavannahGame::CheckFork(const HavannahBoard &board, Piece::Color playerColor)
{
...
bool HavannahGame::CheckRing(Board::Position position, const HavannahBoard &board)
{
    //TODO MORE
    bool ok = false;
...
bool HavannahGame::CheckIfWon(const HavannahBoard &board, Piece::Color playerColor)
{
    if(BFS_Bridge(board, playerColor)==true || CheckFork(board, playerColor)==true)
    {
        return true;
    }
    return false;
