...
bool YGame::BFS_CornerToOpositEdge(const YBoard &board, Piece::Color playerColor)
{
...
bool YGame::VerifyStarts(const YBoard &board, std::vector<Board::Position> &corners, Piece::Color playerColor)
{
    bool verify = false;
...
bool YGame::VerifyEnds(const YBoard &board, std::vector<Board::Position> &endEdge, Board::Position start, Piece::Color playerColor)
{
...
std::vector<Board::Position> YGame::FindPieceOnEdge(int line, int column, int edge, Piece::Color playerColor)
{
...
bool YGame::BFS_AllEdges(const YBoard &board, Piece::Color playerColor)
{
...
bool YGame::CheckIfWon(const YBoard &board, Piece::Color playerColor)
{
    if(BFS_CornerToOpositEdge(board, playerColor)==true || BFS_AllEdges(board, playerColor)==true)
    {
        return true;
    }
    return false;
...
void YGame::Run()
{
    ...
    //namePlayer1=YWindow::nameP1.toStdString();
    //namePlayer2=YWindow::nameP2.toStdString();

    Player p1("namePlayer1", Piece::Color::BLUE, Player::NrPlayer::FIRST);
    Player p2("namePlayer2", Piece::Color::RED, Player::NrPlayer::SECOND);
    ...
    while(!finish)
    {
        //player1's turn
        //check if won
        if (CheckIfWon(YFrame::yBoard,p1.GetColor()))
        {
           //player1 won
           finish=true;
           std::cout<<namePlayer1<<" won!";
        }

        //player2's turn
        //place pice
        if (CheckIfWon(YFrame::yBoard,p2.GetColor()))
        {
            //player2 won
            finish=true;
            std::cout<<namePlayer2<<" won!";
        }
     }
}
